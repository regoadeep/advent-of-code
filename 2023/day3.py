...
def check_gear_ratio(engine_schema: list, start_row_number: int, start_column_number: int, total_rows: int, total_columns: int) -> bool:
    if row_number+2 > rows:
        row_end = rows
    else:
        row_end = row_number+2
    ...
    valid_symbols = "*"
    for i in range(start_row_number-1, row_end):
        for j in range(start_column_number-1, colum_end):
            char = engine_schema[i][j]
            if i >= 0 and j >= 0:
                if (char in valid_symbols):
                    pass
...
for row_index in range(rows):
    ...
    for column_index in range(columns):
        ...
        if char.isdigit():
            number += char
            engine_schematic[row_index][column_index] = "."
            if flag == 1 and len(number) == 1:
                start_row = row_index
                start_column = column_index
        else:
            gear_ratio = check_gear_ratio(
                engine_gear_ratio, start_row, start_column, rows, columns)


print(f"Day 3 part 2 total: {day3part2total}")
